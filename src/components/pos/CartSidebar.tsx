...
export function CartSidebar({ onOpenCheckout, isMobileDrawer, onCloseMobile }: CartSidebarProps) {
  ...
  useEffect(() => {
    // Generate a clean draft order number suffix for display
    const randomSuffix = Math.floor(1000 + Math.random() * 9000);
    setOrderNo(`#ORD-${randomSuffix}`);
  }, []);
  ...
  return (
    <div className="w-full lg:w-[420px] bg-white border-l border-slate-200 flex flex-col h-full shadow-lg shrink-0 overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-100 bg-slate-50/80 space-y-3 shrink-0">
        ...
        {/* Display Order Number (Replaces Table & Guest Number) */}
        <div className="bg-white rounded-xl p-2.5 flex items-center justify-between border border-slate-200 shadow-xs">
          ...
          <div className="flex items-center gap-1 font-mono font-extrabold text-xs text-blue-600 bg-blue-50 px-3 py-1 rounded-lg border border-blue-200/60">
            <Hash className="w-3 h-3 text-blue-500" />
            <span>{orderNo || '#ORD-DRAFT'}</span>
          </div>
        </div>
      </div>
