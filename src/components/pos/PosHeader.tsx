...
export function PosHeader() {
  ...
  return (
    <header className="bg-slate-900 text-white px-4 py-3 flex items-center justify-between shadow-md border-b border-slate-800">
      {/* Brand & Logo */}
      <div className="flex items-center gap-3">
        <div className="p-2 bg-blue-600 rounded-xl shadow-lg shadow-blue-500/30 flex items-center justify-center">
          <Utensils className="w-5 h-5 text-amber-300" />
        </div>
        <div>
          <h1 className="text-base font-bold tracking-wide flex items-center gap-2">
            Kasir Point of Sale
            <span className="text-[10px] bg-blue-500/20 text-blue-300 border border-blue-400/30 px-2 py-0.5 rounded-full uppercase tracking-wider font-semibold">
              WEB POS
            </span>
          </h1>
      ...
      <div className="flex items-center gap-3">
        ...
        {isAdmin && (
          ...
          >
            ...
            <span className="hidden sm:inline">Back-Office ERP</span>
          </Link>
        )}

        <button
          onClick={() => signOut({ callbackUrl: '/login' })}
          className="flex items-center gap-1.5 px-3 py-2 bg-rose-600/10 hover:bg-rose-600/20 text-rose-400 text-xs font-semibold rounded-xl border border-rose-500/30 transition"
          title="Keluar / Logout"
        >
